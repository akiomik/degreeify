...
- Key inference, for charts that state no key. Every key is scored on how many
  of the chart's chords it accounts for, and on whether the chart opens and
  closes on its tonic, which is the only thing that separates a key from its
  relative. Where nothing chooses between the leading candidates the guess is
  declined, leaving the chart in chord names rather than relabelling it
  against the wrong tonic.
